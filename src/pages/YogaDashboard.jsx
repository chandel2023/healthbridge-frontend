...
const YogaDashboard = () => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/yoga/classes', {
        headers: {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`http://localhost:5000/api/yoga/classes/${id}/status`, { status }, {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/yoga/classes/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchClasses();
    } catch (err) {
